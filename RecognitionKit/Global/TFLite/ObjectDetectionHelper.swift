...
struct Result {
    let inferenceTime: Double //辨識時間
    let detections: [Detection]//辨識結果
}
...
class ObjectDetectionHelper: NSObject {
    ...
    init?(modelFileInfo: FileInfo, threadCount: Int, scoreThreshold: Float, maxResults: Int) {
        ...
        guard let frameworkBundle = Bundle(identifier: "com.solidyear.RecognitionKit"),
              let modelPath = frameworkBundle.path(forResource: modelFileInfo.name, ofType: modelFileInfo.extension) else {
    ...
    func detect(picture:UIImage)->  Result?{
        guard let mlImage = MLImage(image: picture) else { return nil }
